...
<!1::OpenOrShowAppBasedOnExeName("Slack.exe")
<!3::OpenOrShowAppBasedOnExeName("firefox.exe")
<!4::OpenOrShowAppBasedOnExeName("Code.exe")
<#m::OpenOrShowAppBasedOnExeName("Plexamp.exe")




; macOS-style alt-` scrolling through open windows of the current application
!`::
...
; macOS-style shift-alt-` scrolling _backwards_ through open windows of the current application
+!`::
WinGet, ActiveProcess, ProcessName, A
WinGet, OpenWindowsAmount, Count, ahk_exe %ActiveProcess%
...
Else
	{
		...
		If WindowsWithSameTitleList > 1 ; If several Window of same type (title checking) exist
		{
			WinSet, Bottom,, % "ahk_id " WindowsWithSameTitleList1  ; Send current window to bottom
			WinActivate, % "ahk_id" WindowsWithSameTitleList2       ; Focus last window
		}
	}
Return
